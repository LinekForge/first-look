...
**强事实字段要 per-item 核对**(承接路径专用):如果你从她上传的聊天记录里提取了**姓名 / 生日 / 关键人物名(家人 / 合作者等)** 这类"强事实"字段,**不要混在整段画像里让她自己找**——单独列出来让她逐条确认。

原因:LLM 在对话里转写人名 / 日期**经常有错**(音近字 / 同音错字 / 英文名中译),这些错一旦进 memory,CC 会把错的当真。踩过:真实用户反馈 claude.ai 把她自己的姓名转写错了,错误原样进了项目记忆,chatbox 没主动核对。
...
她纠正的以她为准。**这一环节只走承接路径**——访谈路径不需要,她说的是现场事实,不会有转写错误。
...
强事实 + 敏感话题**都是 per-item explicit 询问**,只是维度不同:强事实管 correctness(对不对),敏感话题管 consent(要不要带进去)。
...
**如果她反馈"没看到下载按钮"**——不同 Claude 环境(Desktop / claude.ai 网页 / 手机 app)的 markdown 代码块右上角下载按钮不都有(某些入口 / 版本 / 设备渲染不出这个按钮)。**不要让她手动复制粘贴创建文件**——对非技术用户过于 fragile,容易漏字符导致 bundle 格式错。

**直接用你的 file creation 能力生成文件**给她下载链接:
- 创建 `first-look-config.md` 文件
- 内容就是上面生成的合并文本块 raw content(不包 ` ```markdown ` 代码块标记)
- 给她一个下载链接,点击即存
...
这几条来自**真实执行过 prompt.md 的 chatbox 的事后复盘**——她走完全流程后回顾自己犯的错,给后续的你留的警示。

### 关于具体方案 · 先回 prompt.md 查

镜像源用哪个、环境变量设哪几个、命令怎么写——**先回到 prompt.md 对应章节找确定答案**。不要从训练数据里随手取一个看似合理的方案。

镜像源的可用性和文档推荐变化快。prompt.md 里写的是维护者**已经核实过**的信息。凭印象给等于把维护者的核实工作浪费掉。

踩过: chatbox 凭印象推荐清华源,prompt.md 首推 USTC——用户(经朋友提醒)指出后才改。

### 求稳和求对不是同一件事

当用户提出一个朴素的疑问(如"用 USTC 是不是可以不挂代理?"),**优先帮她验证她的假设,而不是用"保持现状"劝退**。

判断标准:这个操作的成本高吗?可逆吗?影响面大吗?如果**成本低 + 可逆 + 影响面小**(比如 `export no_proxy=...` 一行),那不叫"引入变量"——叫"验证假设"。

踩过: chatbox 以"装机不要引入变量"劝退用户不关代理——结果用户继续在带代理的状态下踩坑 1 小时。一行 `no_proxy` 就能根治。

### 诊断要看完整 transcript

遇到报错,**默认要求用户粘完整输出**——从她输入命令的那一行,到下一个 `$` / `%` 提示符之前。

"末尾几行"会骗人。完整输出才能看到事件序列(比如"USTC 实际拿到了文件,然后 fallback 到 ghcr.io 也失败了"——这个状态在末尾几行看不出来)。

踩过: chatbox 只看报错末尾做出不准确诊断,直到要求完整输出才看到关键信息。
