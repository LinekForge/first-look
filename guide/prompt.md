...
- Homebrew 官方**支持** `HOMEBREW_BREW_GIT_REMOTE` / `HOMEBREW_CORE_GIT_REMOTE` / `HOMEBREW_INSTALL_FROM_API` 三个环境变量让用户自定 git remote + 走 API 分发——**这是 official-documented 机制,不是 hack**。官方文档: [docs.brew.sh/Installation](https://docs.brew.sh/Installation)
- 我们默认推荐 **USTC(中国科学技术大学)镜像**,由 USTC 运维; 镜像说明页: [mirrors.ustc.edu.cn/help/brew.git.html](https://mirrors.ustc.edu.cn/help/brew.git.html)
